...
import { PauseOutlined, PlayCircleOutlined, MutedOutlined, SoundOutlined, ArrowsAltOutlined, ReloadOutlined, ShrinkOutlined, FullscreenExitOutlined, FullscreenOutlined } from '@ant-design/icons';
...
const LiveFeedViewer = ({ stream, className, title = 'Live Feed Viewer', subtitle = 'Draw polygons (lines only) on live video', defaultEnabled = true, enableMultiplePolygons = true, initialPolygons, onPolygonsChange, onPolygonDetails, showControls, }) => {
    ...
    return (jsxs(Card, { className: cn('w-full h-full', className), bodyStyle: { padding: 16 }, children: [jsxs("div", { className: "flex items-center justify-between mb-3", children: [jsxs("div", { children: [jsx(Text, { strong: true, className: "block", children: title }), jsx(Text, { type: "secondary", className: "text-xs", children: subtitle })] }), jsxs("div", { className: "flex items-center gap-3", children: [controlsVisible.viewer && (jsxs("div", { className: "flex items-center gap-2", children: [jsx(Text, { className: "text-xs", children: "Viewer" }), jsx(Switch, { checked: enabled, onChange: setEnabled })] })), controlsVisible.draw && (jsxs("div", { className: "flex items-center gap-2", children: [jsx(Text, { className: "text-xs", children: "Draw" }), jsx(Switch, { checked: drawingEnabled, onChange: setDrawingEnabled })] })), controlsVisible.reset && (jsx(Button, { size: "small", onClick: handleReset, disabled: !enabled, icon: jsx(ReloadOutlined, {}), children: "Reset" })), controlsVisible.fullscreen && (jsx(Tooltip, { title: isFullscreen ? 'Exit Fullscreen' : 'Enter Fullscreen', children: jsx(Button, { size: "small", onClick: toggleFullscreen, icon: isFullscreen ? jsx(FullscreenExitOutlined, {}) : jsx(FullscreenOutlined, {}) }) }))] })] }), jsx("div", { ref: containerRef, className: "relative w-full overflow-hidden rounded-md bg-black", style: { aspectRatio: '16 / 9' }, children: enabled ? (jsxs(Fragment, { children: [jsx(VideoPlayer, { stream: stream, autoPlay: true, muted: true, controls: false, className: "w-full h-full" }), jsx("canvas", { ref: canvasRef, className: "absolute inset-0", style: { pointerEvents: canDraw ? 'auto' : 'none', cursor: canDraw ? 'crosshair' : 'default' }, onClick: handleCanvasClick })] })) : (jsx("div", { className: "absolute inset-0 flex items-center justify-center", children: jsxs("div", { className: "text-center text-gray-300", children: [jsx("div", { className: "text-2xl mb-2", children: "Viewer is OFF" }), jsx("div", { className: "text-sm opacity-80", children: "Toggle ON to start live feed" })] }) })) })] }));
};
